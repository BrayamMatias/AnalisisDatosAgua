...
# Filtrar los datos y eliminar los outliers
data_no_outliers = data[~((data < lower_bound) | (data > upper_bound)).any(axis=1)]

# Crear una copia del conjunto de datos sin outliers y ocultar un porcentaje de los valores (por ejemplo, 10%)
np.random.seed(0)  # Para reproducibilidad
mask = np.random.rand(*data_no_outliers[numeric_cols].shape) < 0.1  # Ocultar el 10% de los datos
data_with_missing = data_no_outliers[numeric_cols].mask(mask)

# Definir un modelo de KNN
def train_knn(X_train, y_train, n_neighbors=24):
...
for index, row in data_imputed.iterrows():
    ...
    if row.isna().all():
        ...
        for col in numeric_cols:
            row[col] = data_no_outliers[col].mean()
    else:
        while row.isna().any():  # Continuar hasta que no haya valores faltantes
            ...
            X_train = data_no_outliers[available_cols]
            y_train = data_no_outliers[target_col]
            ...
            if len(X_train) < 5:
                row[target_col] = data_no_outliers[target_col].mean()
            else:
...
mse = mean_squared_error(data_no_outliers[numeric_cols][mask], data_imputed[numeric_cols][mask])
rmse = np.sqrt(mse)
mae = mean_absolute_error(data_no_outliers[numeric_cols][mask], data_imputed[numeric_cols][mask])
r2 = r2_score(data_no_outliers[numeric_cols][mask], data_imputed[numeric_cols][mask])
...
data_imputed.insert(0, 'SITIO', sitio.loc[data_no_outliers.index])
data_imputed.to_csv('Analisis Predictivo\\Imputacion\\KNN\\data_select_iterative_imputed_KNN_no_outliers.csv', index=False)
